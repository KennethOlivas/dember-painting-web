"use client";
import {
  directions,
  directionsLink,
  email,
  phone,
  whatsapp,
} from "@/constants/contact/ContactInfo";
import { Card, Input, Link, Textarea } from "@nextui-org/react";
import React from "react";
import NextLink from "next/link";

const Contact = () => {
  return (
    <section id="contact" className="relative py-20 md:py-[120px]">
      <div className="absolute left-0 top-0 z-[-1] h-1/2 w-full bg-primary lg:h-[45%] xl:h-1/2" />
      <div className="flex flex-wrap items-center 2xl:px-48 md:px-12 sm:px-12 px-0">
        <div className="w-full px-4 lg:w-7/12 xl:w-8/12">
          <>
            <div className=" mb-12 lg:mb-[150px]">
              <span className="font-headingFont text-secondary mb-5 text-base font-semibold">
                CONTACT US
              </span>
              <h2 className="text-4xl text-white font-semibold">
                Let&apos;s talk about <br />
                Love to hear from you!
              </h2>
            </div>

            <main className="mb-12 grid grid-cols-1 xl:grid-cols-3 gap-4">
              <section className="mb-8 col-span-1 max-w-full lg:mt-24 xl:mt-0">
                <div className="flex">
                  <div className="text-primary mr-6 text-[32px]">
                    <svg
                      width={29}
                      height={35}
                      viewBox="0 0 29 35"
                      className="fill-current text-white lg:text-primary">
                      <path d="M14.5 0.710938C6.89844 0.710938 0.664062 6.72656 0.664062 14.0547C0.664062 19.9062 9.03125 29.5859 12.6406 33.5234C13.1328 34.0703 13.7891 34.3437 14.5 34.3437C15.2109 34.3437 15.8672 34.0703 16.3594 33.5234C19.9688 29.6406 28.3359 19.9062 28.3359 14.0547C28.3359 6.67188 22.1016 0.710938 14.5 0.710938ZM14.9375 32.2109C14.6641 32.4844 14.2812 32.4844 14.0625 32.2109C11.3828 29.3125 2.57812 19.3594 2.57812 14.0547C2.57812 7.71094 7.9375 2.625 14.5 2.625C21.0625 2.625 26.4219 7.76562 26.4219 14.0547C26.4219 19.3594 17.6172 29.2578 14.9375 32.2109Z" />
                      <path d="M14.5 8.58594C11.2734 8.58594 8.59375 11.2109 8.59375 14.4922C8.59375 17.7188 11.2187 20.3984 14.5 20.3984C17.7812 20.3984 20.4062 17.7734 20.4062 14.4922C20.4062 11.2109 17.7266 8.58594 14.5 8.58594ZM14.5 18.4297C12.3125 18.4297 10.5078 16.625 10.5078 14.4375C10.5078 12.25 12.3125 10.4453 14.5 10.4453C16.6875 10.4453 18.4922 12.25 18.4922 14.4375C18.4922 16.625 16.6875 18.4297 14.5 18.4297Z" />
                    </svg>
                  </div>
                  <div>
                    <h5 className="mb-6 text-lg font-semibold text-white lg:text-black">
                      Our Location
                    </h5>
                    <Link
                      as={NextLink}
                      target="_blank"
                      href={directionsLink}
                      className="text-white lg:text-primary">
                      {directions}
                    </Link>
                  </div>
                </div>
              </section>
              <section className="mb-8 col-span-1 max-w-full">
                <div className="flex">
                  <div className="text-primary mr-6 text-[32px]">
                    <svg
                      width={29}
                      height={35}
                      xmlns="http://www.w3.org/2000/svg"
                      fill="none"
                      viewBox="0 0 24 24"
                      strokeWidth={1.5}
                      stroke="currentColor"
                      className=" text-white lg:text-primary">
                      <path
                        strokeLinecap="round"
                        strokeLinejoin="round"
                        d="M2.25 6.75c0 8.284 6.716 15 15 15h2.25a2.25 2.25 0 002.25-2.25v-1.372c0-.516-.351-.966-.852-1.091l-4.423-1.106c-.44-.11-.902.055-1.173.417l-.97 1.293c-.282.376-.769.542-1.21.38a12.035 12.035 0 01-7.143-7.143c-.162-.441.004-.928.38-1.21l1.293-.97c.363-.271.527-.734.417-1.173L6.963 3.102a1.125 1.125 0 00-1.091-.852H4.5A2.25 2.25 0 002.25 4.5v2.25z"
                      />
                    </svg>
                  </div>
                  <div>
                    <h5 className="mb-6 text-lg font-semibold text-white lg:text-black">
                      Phone Number
                    </h5>
                    <Link
                      as={NextLink}
                      target="_blank"
                      href={whatsapp}
                      className="text-white lg:text-primary">
                      {phone}
                    </Link>
                  </div>
                </div>
              </section>
              <section className="mb-8 col-span-1  max-w-full">
                <div className="flex">
                  <div className="text-primary mr-6 text-[32px]">
                    <svg
                      width={34}
                      height={25}
                      viewBox="0 0 34 25"
                      className="fill-current text-white lg:text-primary">
                      <path d="M30.5156 0.960938H3.17188C1.42188 0.960938 0 2.38281 0 4.13281V20.9219C0 22.6719 1.42188 24.0938 3.17188 24.0938H30.5156C32.2656 24.0938 33.6875 22.6719 33.6875 20.9219V4.13281C33.6875 2.38281 32.2656 0.960938 30.5156 0.960938ZM30.5156 2.875C30.7891 2.875 31.0078 2.92969 31.2266 3.09375L17.6094 11.3516C17.1172 11.625 16.5703 11.625 16.0781 11.3516L2.46094 3.09375C2.67969 2.98438 2.89844 2.875 3.17188 2.875H30.5156ZM30.5156 22.125H3.17188C2.51562 22.125 1.91406 21.5781 1.91406 20.8672V5.00781L15.0391 12.9922C15.5859 13.3203 16.1875 13.4844 16.7891 13.4844C17.3906 13.4844 17.9922 13.3203 18.5391 12.9922L31.6641 5.00781V20.8672C31.7734 21.5781 31.1719 22.125 30.5156 22.125Z" />
                    </svg>
                  </div>
                  <div>
                    <h5 className="mb-6 text-lg font-semibold text-white lg:text-black">
                      How Can We Help?
                    </h5>
                    <Link
                      href={`mailto:${email}`}
                      target="_blank"
                      as={NextLink}
                      className="text-white lg:text-primary">
                      {email}
                    </Link>
                  </div>
                </div>
              </section>
            </main>
          </>
        </div>
        <Card className="w-full px-4 lg:w-5/12 xl:w-4/12">
          <div
            className="shadow-testimonial wow fadeInUp rounded-lg bg-white px-8 py-10 sm:px-10 sm:py-12 md:p-[60px] lg:p-10 lg:px-10 lg:py-12 2xl:p-[60px]"
            data-wow-delay=".2s">
            <h3 className="mb-8 text-2xl font-semibold md:text-[26px]">
              Send us a Message
            </h3>
            <form>
              <div className="mb-6">
                <Input
                  type="text"
                  label="Full name"
                  variant="underlined"
                  color="primary"
                />
              </div>
              <div className="mb-6">
                <Input
                  type="enail"
                  label="Email"
                  variant="underlined"
                  color="primary"
                />
              </div>
              <div className="mb-6">
                <Input
                  type="text"
                  label="Full name"
                  variant="underlined"
                  color="primary"
                />
              </div>
              <div className="mb-6">
                <Textarea
                  variant="underlined"
                  labelPlacement="outside"
                  placeholder="Enter your message here"
                  className="col-span-12 md:col-span-6 mb-6 md:mb-0"
                />
              </div>
              <div className="mb-0">
                <button
                  type="submit"
                  className="bg-primary hover:bg-dark inline-flex items-center justify-center rounded px-6 py-4 text-base font-medium text-white transition duration-300 ease-in-out">
                  Send Message
                </button>
              </div>
            </form>
          </div>
        </Card>
      </div>
    </section>
  );
};

export default Contact;
